...
use coprocessor::requests::{CalibrateRequest, GetPositionRequest, OtosPosition, SetOffsetsRequest, SetPositionRequest};
use evian::{
        ...
        loop {
            if let Ok(controller) = self.controller.state() {
                ...
                if controller.button_a.is_now_pressed() {
                    ...
                    // }))).detach();
                    _ = dbg!(self.coprocessor.send_request(SetOffsetsRequest(OtosPosition {
                        x: 0.0 * Inches,
                        y: 0.0 * Inches,
                        heading: 90.0 * Degrees,
                    })).await);
    ...
    async fn autonomous(&mut self) {
        loop {
            ...
            if let Ok(position) = position {
                self.ui.app().global::<OdometryPageState>().set_position(Pose {
